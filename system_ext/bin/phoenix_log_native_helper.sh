...
MTK_KE_FILE_PATTERN="AEE_SYSTEM_LAST_KMSG"
...
function collect_last_kmsg_log()
{
    tmp_dir=$1

    if [ "x${HARDWARE}" == "xqcom" ]; then
        last_kmsg_log=$(search_latest_file_with_pattern ${MINIDUMP_DIR} ${KE_FILE_PATTERN})
        if [ "x${last_kmsg_log}" != "x" ]; then
            cp -a  ${MINIDUMP_DIR}/${last_kmsg_log} ${tmp_dir}/${last_kmsg_log}
        fi
    else
        last_kmsg=$(search_latest_file_with_pattern ${AEE_DB_DIR} ${MTK_KE_FILE_PATTERN})
        if [ "x${last_kmsg}" != "x" ]; then
            cp -a  ${AEE_DB_DIR}/${last_kmsg} ${tmp_dir}/${last_kmsg}
        fi
    fi
}
