...
function countCachedHciLog() {
    ...
    cachedHciLogByteCnt=`ls -Al ${hciLogCachedPath} | grep -E "btsnoop|CsLog" | awk 'BEGIN{sum7=0}{sum7+=$5}END{print sum7}'`
    cachedHciLogNumCnt=`ls -l ${hciLogCachedPath}  | grep -E "btsnoop|CsLog" | wc -l`
    enHciLogCnt=`ls -Al $enPath | awk 'BEGIN{sum7=0}{sum7+=$5}END{print sum7}'`
    ...
    if [ cachedHciLogByteCnt -gt $threadshold ] || [ cachedHciLogNumCnt -gt 20 ];then
        deleteCachedHciLog $hciLogCachedPath "btsnoop|CsLog"
    fi

    deHciLogCnt=`ls -Al $dePath | awk 'BEGIN{sum7=0}{sum7+=$5}END{print sum7}'`
...
function deleteCachedHciLog() {
    ...
    then
        ...
        filelist=`ls -Atr $logPath | grep -E $2`
    fi
