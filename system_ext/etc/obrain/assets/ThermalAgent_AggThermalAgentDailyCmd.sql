...
from (
	...
	from (
		select json_object('typ', zone_name, 'mode', screen_mode || thermal_mode || power_mode
		       , 'm_tmp', max_temp, 'ntyp', network_type, 'u_td', up_trend, 'd_td', down_trend, 'k_td', keep_trend, 'm_rt', max_rate, 'tmp', json_array(VectorI('zone$0', ThermalList))) as oneline
		from (
		...
		from (
			select json_object('app', obfuscate(fg_app), 'typ', zone_name, 'mode', screen_mode || thermal_mode || power_mode
			      , 'm_tmp', max_temp, 'ntyp', network_type, 'u_td', up_trend, 'd_td', down_trend, 'k_td', keep_trend, 'm_rt', max_rate, 'tmp', json_array(VectorI('zone$0', ThermalList))) as oneline
			from (
