...
, SERVICE_CALLER_MERGED_ONECOLUME_TABLE as (
    select '{' || '"srv":"' || service_name || '", '
    || '"spn":"' || service_proc_name || '",'
    || '"tt_tm":' || total_calltimes || ','
    || '"cun":' || group_concat(caller_uid_names) || ','
    || '"cpn":' || group_concat(caller_proc_names) || ','
    || '"ctn":' || group_concat(caller_thread_names) || ','
	|| '"s_md":' || group_concat(screen_modes) || ','
	|| '"g_md":' || group_concat(ground_modes) || ','
    || '"c_tm":' || group_concat(caller_calltimes) || '}' as service_stats
