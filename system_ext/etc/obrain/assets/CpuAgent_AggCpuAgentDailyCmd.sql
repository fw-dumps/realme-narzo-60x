...
CPU_FG_EG_TBL_TMP as (
    select "{{" || '"mode":' || '"' || screen_mode || thermal_mode || power_mode || '",'
        || MapSumVectorJsonFormatII('"c$0_du"', 'sum(c$0_$1_du)', CpuClusterList, CpuFreqListList) || ","
        || MapVectorJsonFormatII('"c$0_fdu"', 'sum(c$0_$1_du)', CpuClusterList, CpuFreqListList) || ","
        || '"pmu":' || '"' || sum(pmu_total_eg) || '",'
        || '"w_du":' || '"' || (PARAM_VAR.var_end_ts - PARAM_VAR.var_start_ts) || '",'
...
PWR_TBL as (
    select '{{' ||
        ...
        || '"lkg":' || PARAM_VAR.var_leakage || ','
        || '"rvs":' || PARAM_VAR.var_revision ||'}}' as power_table
    from PARAM_VAR
)
----------------------------------------------------------------------------------------------------
