...
cat /sys/kernel/helio-dvfsrc/dvfsrc_opp_table
...
do_ddr_vcorefs_random(){
	...
	echo " ***** STARTING DVFS STRESS ***** "
	echo 1 > /sys/kernel/helio-dvfsrc/dvfsrc_enable
	...
	do
		...
		if [ $fix_opp -eq 24 ]; then
			fix_opp=23
		fi
		echo $fix_opp > /sys/kernel/helio-dvfsrc/dvfsrc_force_vcore_dvfs_opp
		cat /sys/kernel/helio-dvfsrc/dvfsrc_dump | grep -e uv -e khz
	done
}
...
do_ddr_vcorefs_max(){
	...
	echo 0 > /sys/kernel/helio-dvfsrc/dvfsrc_enable
	for i in $(seq 1 ${ntest})
	do
		sleep $T_DVFS_INTERVAL
		cat /sys/kernel/helio-dvfsrc/dvfsrc_dump | grep -e uv -e khz
...
do_ddr_vcorefs_min(){
	...
	echo 28 > /sys/kernel/helio-dvfsrc/dvfsrc_force_vcore_dvfs_opp
