...
function configure_read_ahead_kb_values() {
	...
	if [ -f /sys/block/sdc/queue/read_ahead_kb ]; then
		echo $ra_kb > /sys/block/sdc/queue/read_ahead_kb
	fi
...
function oplus_configure_tuning_swappiness() {
	...
	if [ $MemTotal -le 4194304 ]; then
		...
		echo 850 > $para_path/swappiness_threshold2_size
	elif [ $MemTotal -le 6291456 ]; then
...
function configure_memory_parameters() {
	...
	# modify by wangzhipeng 20230823 start fix min_free_kbytes
	if [ $MemTotal -le 4194304 ]; then
		echo 8700 > /proc/sys/vm/min_free_kbytes
	fi
	# modify by wangzhipeng 20230823 end fix min_free_kbytes
